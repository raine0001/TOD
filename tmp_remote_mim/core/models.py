...
class StudioProject(Base, TimestampMixin):
    ...
    owner: Mapped[str] = mapped_column(String(120), default="Dave + MIM", index=True)
    health: Mapped[str] = mapped_column(String(80), default="good", index=True)
    why_it_matters: Mapped[str] = mapped_column(Text, default="")
    origin_story: Mapped[str] = mapped_column(Text, default="")
    next_action: Mapped[str] = mapped_column(Text, default="")
    dave_needed: Mapped[bool] = mapped_column(Boolean, default=False, index=True)
    metadata_json: Mapped[dict] = mapped_column(JSON, default=dict)


class StudioProjectSignal(Base, TimestampMixin):
    __tablename__ = "studio_project_signals"

    id: Mapped[int] = mapped_column(primary_key=True)
    title: Mapped[str] = mapped_column(String(220), index=True)
    signal_type: Mapped[str] = mapped_column(String(80), default="observation", index=True)
    status: Mapped[str] = mapped_column(String(80), default="observation", index=True)
    priority: Mapped[str] = mapped_column(String(40), default="normal", index=True)
    source_surface: Mapped[str] = mapped_column(String(120), default="studio", index=True)
    source_text: Mapped[str] = mapped_column(Text, default="")
    why_it_matters: Mapped[str] = mapped_column(Text, default="")
    suggested_action: Mapped[str] = mapped_column(String(80), default="review", index=True)
    project_id: Mapped[int | None] = mapped_column(
        ForeignKey("studio_projects.id", ondelete="SET NULL"),
        nullable=True,
        index=True,
    )
    metadata_json: Mapped[dict] = mapped_column(JSON, default=dict)
...
class StudioProjectEvent(Base, TimestampMixin):
    __tablename__ = "studio_project_events"
    ...
    actor: Mapped[str] = mapped_column(String(120), default="MIM", index=True)
...
class StudioProjectLink(Base, TimestampMixin):
    __tablename__ = "studio_project_links"
    ...
    relation: Mapped[str] = mapped_column(String(80), default="related", index=True)
    label: Mapped[str] = mapped_column(String(220), default="")
    metadata_json: Mapped[dict] = mapped_column(JSON, default=dict)


class StudioDocument(Base, TimestampMixin):
    __tablename__ = "studio_documents"

    id: Mapped[int] = mapped_column(primary_key=True)
    title: Mapped[str] = mapped_column(String(260), index=True)
    summary: Mapped[str] = mapped_column(Text, default="")
    document_type: Mapped[str] = mapped_column(String(80), default="note", index=True)
    category: Mapped[str] = mapped_column(String(120), default="library", index=True)
    status: Mapped[str] = mapped_column(String(80), default="active", index=True)
    owner: Mapped[str] = mapped_column(String(120), default="Dave + MIM", index=True)
    created_by: Mapped[str] = mapped_column(String(120), default="MIM", index=True)
    source_kind: Mapped[str] = mapped_column(String(80), default="manual", index=True)
    ...
    preserve_policy: Mapped[str] = mapped_column(String(80), default="reference", index=True)
    snapshot_status: Mapped[str] = mapped_column(String(80), default="not_requested", index=True)
    content_text: Mapped[str] = mapped_column(Text, default="")
    tags_json: Mapped[list[str]] = mapped_column(JSON, default=list)
    metadata_json: Mapped[dict] = mapped_column(JSON, default=dict)


class StudioDocumentLink(Base, TimestampMixin):
    __tablename__ = "studio_document_links"
...
class StudioReportCanvas(Base, TimestampMixin):
    __tablename__ = "studio_report_canvases"

    id: Mapped[int] = mapped_column(primary_key=True)
    title: Mapped[str] = mapped_column(String(260), index=True)
    prompt: Mapped[str] = mapped_column(Text, default="")
    dataset_key: Mapped[str] = mapped_column(String(120), default="studio_overview", index=True)
    ...
    actions_json: Mapped[list[dict]] = mapped_column(JSON, default=list)
    metadata_json: Mapped[dict] = mapped_column(JSON, default=dict)
...
class ProjectPortalAccount(Base, TimestampMixin):
    ...
    locations: Mapped[str] = mapped_column(String(80), default="")
    website: Mapped[str] = mapped_column(String(300), default="")
    metadata_json: Mapped[dict] = mapped_column(JSON, default=dict)


class ProjectPortalProject(Base, TimestampMixin):
    __tablename__ = "project_portal_projects"

    id: Mapped[int] = mapped_column(primary_key=True)
    account_id: Mapped[int | None] = mapped_column(
        ForeignKey("project_portal_accounts.id", ondelete="SET NULL"), nullable=True
    )
    title: Mapped[str] = mapped_column(String(200), index=True)
    status: Mapped[str] = mapped_column(String(80), default="Draft")
    summary: Mapped[str] = mapped_column(Text, default="")
    mode: Mapped[str] = mapped_column(String(80), default="guided")
    readiness_json: Mapped[dict] = mapped_column(JSON, default=dict)
    metadata_json: Mapped[dict] = mapped_column(JSON, default=dict)


class ProjectPortalDiscoverySession(Base, TimestampMixin):
    __tablename__ = "project_portal_discovery_sessions"

    id: Mapped[int] = mapped_column(primary_key=True)
    project_id: Mapped[int] = mapped_column(
        ForeignKey("project_portal_projects.id", ondelete="CASCADE"), index=True
    )
    mode: Mapped[str] = mapped_column(String(80), default="guided")
...
class PublicVisitEvent(Base, TimestampMixin):
    __tablename__ = "public_visit_events"

    id: Mapped[int] = mapped_column(primary_key=True)
    visitor_hash: Mapped[str] = mapped_column(String(96), default="", index=True)
    ip_hash: Mapped[str] = mapped_column(String(96), default="", index=True)
    ...
    event_type: Mapped[str] = mapped_column(String(80), default="page_view", index=True)
...
class ProjectPortalBlueprint(Base, TimestampMixin):
    __tablename__ = "project_portal_blueprints"

    id: Mapped[int] = mapped_column(primary_key=True)
    project_id: Mapped[int] = mapped_column(
        ForeignKey("project_portal_projects.id", ondelete="CASCADE"), index=True
    )
    discovery_session_id: Mapped[int | None] = mapped_column(
        ForeignKey("project_portal_discovery_sessions.id", ondelete="SET NULL"),
        nullable=True,
    )
    business_problem: Mapped[str] = mapped_column(Text, default="")
    desired_outcome: Mapped[str] = mapped_column(Text, default="")
    proposed_modules_json: Mapped[list] = mapped_column(JSON, default=list)
    data_sources_json: Mapped[list] = mapped_column(JSON, default=list)
    integrations_json: Mapped[list] = mapped_column(JSON, default=list)
