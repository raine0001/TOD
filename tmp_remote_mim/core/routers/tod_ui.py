...
def _classify_prompt(message: str) -> str:
    ...
    if "copilot-style" in text or "handoff" in text or "package the current issue" in text:
        return "handoff"
    if "training" in text and ("start" in text or "run" in text or "loop" in text):
        return "training"
    ...
    if any(token in text for token in ("can you fix", "fix this", "please fix", "troubleshoot", "debug this", "begin to troubleshoot")):
        return "task"
    if any(token in text for token in ("objective", "active task", "implement", "build", "execution loop contract")):
        return "task"
    return "status"
...
def _build_execution_feed_messages(state: dict[str, Any]) -> list[dict[str, str]]:
    ...
    if phase_progress.get("available"):
        ...
        messages.append(
            {
                "role": "system",
                "content": f"Phase 1 progress: {progress_percent}% complete. Next gate: {next_gate}.",
                "created_at": created_at,
            }
        )
        if progress_summary:
...
<body>
  <main class=\"page\">
    <section class=\"shell\">
      ...
      <section class=\"facts\">
        ...
        <article class=\"fact\"><div class=\"fact-label\">Phase 1 Progress</div><div id=\"factPhaseProgress\" class=\"fact-value\">-</div><div id=\"factPhaseProgressMeta\" class=\"fact-meta\">Waiting for bounded execution progress.</div></article>
  ...
  <script>
    ...
    function renderPrimaryStatus(status, execution) {{ const statusPayload = status && typeof status === 'object' ? status : {{}}; const executionPayload = execution && typeof execution === 'object' ? execution : {{}}; const useExecution = Boolean(executionPayload.available) && ['working', 'waiting', 'complete', 'stalled'].includes(safeText(executionPayload.activity_state, 'idle').toLowerCase()); if (useExecution) {{ const phaseProgress = executionPayload.phase_progress && typeof executionPayload.phase_progress === 'object' ? executionPayload.phase_progress : {{}}; const stallSignal = executionPayload.stall_signal && typeof executionPayload.stall_signal === 'object' ? executionPayload.stall_signal : {{}}; const phaseSummary = Boolean(phaseProgress.available) ? `Phase 1 ${{Math.max(0, Math.min(100, Number(phaseProgress.percent_complete || 0)))}}% complete; next gate ${{safeText(phaseProgress.next_gate, 'Unknown')}}.` : ''; const stallSummary = stallSignal.flagged ? safeText(stallSignal.summary, '') : executionPayload.available ? 'Stall watch clear.' : ''; statusChip.textContent = safeText(executionPayload.activity_label, 'UNKNOWN').toUpperCase(); statusChip.dataset.tone = safeText(executionPayload.activity_state, 'unknown').toLowerCase(); statusHeadline.textContent = `TOD execution is ${{safeText(executionPayload.activity_label, 'Unknown').toLowerCase()}}`; statusSummary.textContent = safeText([safeText(executionPayload.activity_summary || executionPayload.summary, ''), phaseSummary, stallSummary].filter(Boolean).join(' '), 'No shared TOD execution summary is available.'); return; }} statusChip.textContent = safeText(statusPayload.label, 'UNKNOWN'); statusChip.dataset.tone = safeText(statusPayload.code, 'unknown').toLowerCase(); statusHeadline.textContent = safeText(statusPayload.headline, 'TOD state unavailable'); statusSummary.textContent = safeText(statusPayload.summary, 'No shared TOD summary is available.'); }}
    function renderTopActivity() {{ const execution = latestExecution && typeof latestExecution === 'object' ? latestExecution : {{}}; const sessionActivity = latestSessionActivity && typeof latestSessionActivity === 'object' ? latestSessionActivity : {{}}; const useExecution = Boolean(execution.available) && safeText(execution.activity_state, 'idle').toLowerCase() !== 'idle'; const state = useExecution ? safeText(execution.activity_state, 'idle').toLowerCase() : safeText(sessionActivity.state, 'idle').toLowerCase(); const label = useExecution ? safeText(execution.activity_label, 'Idle') : safeText(sessionActivity.label, 'Idle'); const summary = useExecution ? safeText(execution.activity_summary, 'Waiting for TOD activity.') : safeText(sessionActivity.summary, 'Waiting for TOD activity.'); const ageSeconds = useExecution ? Number(execution.last_update_age_seconds) : Number(sessionActivity.last_activity_age_seconds); const ageText = Number.isFinite(ageSeconds) && ageSeconds >= 0 ? ` · last update ${{formatSeconds(ageSeconds)}} ago` : ''; if (chatActivityIndicator) chatActivityIndicator.dataset.state = state; if (chatActivityText) chatActivityText.textContent = label; if (chatActivitySummary) chatActivitySummary.textContent = `${{summary}}${{ageText}}`; }}
