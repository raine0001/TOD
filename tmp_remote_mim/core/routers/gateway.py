...
from core.primitive_request_recovery_service import (
    ...
    synchronize_latest_result_artifact_from_dispatch,
)
...
from core.ui_health_service import build_mim_ui_health_snapshot, summarize_runtime_health
    ...
    updated_context.update(
        {
            ...
            "clarification_state": {
                "active": conversation_topic == "action_confirmation",
                "prompt": _compact_text(assistant_text, 240) if assistant_text else "",
                "outcome": str(resolution.outcome or "").strip(),
                "reason": str(resolution.reason or "").strip(),
                "pending_action_request": pending_action_request,
...
async def _compose_conversation_reply(
    *,
    ...
    return {
        "reply_text": str(reply_contract.reply_text or fallback_reply).strip(),
...
async def _build_live_operational_context(
    ...
    runtime_health = await build_mim_ui_health_snapshot(db=db)
    runtime_recovery = runtime_recovery_service.get_summary()
    initiative_status = await build_initiative_status(db=db)
    program_status = (
...
def _build_live_operational_response(
    ...
    def _priority_summary() -> str:
        ...
        if runtime_health:
            return _runtime_health_detail(runtime_health)
        return ""
    ...
    if _mentions_tod(query) and any(
        token in query for token in {"how is", "status", "healthy", "doing"}
    ):
        parts: list[str] = []
        if tod_collaboration:
    ...
    if any(
        ...
        if stability_guard:
            parts.append(f"Stability guard: {stability_guard.rstrip('.').lower()}")
        prefix = "Status check: you asked me to check status. Current status: "
        if "check status" not in query:
            prefix = "Status summary: you asked me to summarize your status. Current status: "
        return _compact_text(
            prefix + "; ".join(parts) + ".",
            320,
        )
    ...
    if any(
        ...
        if stability_guard:
            parts.append(f"Stability guard: {stability_guard.rstrip('.').lower()}")
        return _compact_text(
            "Current health check: you asked me to check your current health now. Current health: "
            + "; ".join(parts)
            + ".",
            320,
        )
    ...
    if any(
        ...
        if runtime_recovery:
            parts.append(runtime_recovery.rstrip('.'))
        return _compact_text("Current status: " + "; ".join(parts) + ".", 280)
...
def _conversation_response(
    ...
    if any(
        ...
        return (
            "You asked what I can do. I can answer questions, explain current status, "
            "suggest a bounded plan, inspect the live runtime, and carry out focused implementation work in this repo. "
            "Ask one direct question or name one concrete task."
        )
    ...
    if any(
        ...
        return "Status summary: you asked me to summarize your status. Current status: online, stable, and focused on reliable MIM to TOD handoff."
    ...
    if (
        ...
        if "health" in normalized_query:
            return "Current health check: you asked me to check your current health now. Current health: I am online and operating normally."
        if "check status" in normalized_query:
            return "Status check: you asked me to check status. Current status: I am online and operating normally."
        return "Current status: you asked for my status. I am online and operating normally."
...
async def _store_normalized(payload: NormalizedInputCreate, db: AsyncSession) -> dict:
    ...
    if event.source == "text" and _looks_like_mim_tod_executable_handoff_request(
    ...
    elif event.source == "text" and not continuation_validation_request and not _looks_like_bounded_choice_decision_prompt(
        event.raw_input
    ) and (
        ...
        or
        _looks_like_bounded_tod_bridge_warning_recommendation_request(
            event.raw_input,
            event.parsed_intent,
            event.safety_flags,
        )
