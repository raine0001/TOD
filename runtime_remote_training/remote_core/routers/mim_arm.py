...
def _latest_readiness(shared_root: Path) -> dict[str, object]:
    ...
    for artifact_name in (TOD_TASK_RESULT_ARTIFACT, TOD_COMMAND_STATUS_ARTIFACT):
        ...
        candidates.append((generated_at, {"artifact_name": artifact_name, "payload": payload, "readiness": readiness}))
    if not candidates:
        return {
            "status": "unknown",
    ...
    readiness = _json_dict(selected.get("readiness"))
    return {
...
def _arm_execution_availability_block(status: dict[str, object], *, action_name: str = "safe_home") -> dict[str, object]:
    if not bool(status.get("arm_online", False)):
        return {
            "active": True,
    ...
    if (
        ...
        and bool(status.get("camera_online", False))
        and str(status.get("tod_execution_block_reason") or "").strip() == "communication_escalation_persistent"
    ):
        return {
            "active": False,
            "reason": "Camera-only capture_frame is allowed past persistent communication escalation when explicitly operator-approved; no arm motion is requested.",
        }
...
def _build_bounded_pose_event_and_resolution(
    *,
    ...
    availability_block = _arm_execution_availability_block(status, action_name=action_name)
