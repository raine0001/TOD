...
try {
    Send-RemoteFileB64 -SessionId $session.SessionId -LocalPath "tmp_remote_mim/core/routers/public_chat.py" -RemotePath "/home/testpilot/mim/core/routers/public_chat.py"
    $verifyCommand = "cd /home/testpilot/mim && python3 -m py_compile core/routers/public_chat.py && grep -n 'Today is.*current_date' core/routers/public_chat.py && grep -n 'what.*day.*week' core/routers/public_chat.py"
    $verify = Invoke-RemoteChecked -SessionId $session.SessionId -Command $verifyCommand -TimeoutSeconds 60
    $verify.Output
}
finally {
