...
$todUiLanProxyScript = Join-Path $PSScriptRoot "tod_ui_lan_proxy.py"
...
function Write-TodUiStartupDiagnostic {
    param(
        ...
        [string]$AdvertiseHostValue = "",
        [string[]]$ListenHosts = @("localhost"),
        [string]$Reason = ""
    )

    $advertiseHostResolved = if ([string]::IsNullOrWhiteSpace($AdvertiseHostValue)) { "localhost" } else { [string]$AdvertiseHostValue }
    $payload = [pscustomobject]@{
        ok = $true
        status = $Status
        port = $PortValue
        listen_hosts = @($ListenHosts | Where-Object { -not [string]::IsNullOrWhiteSpace([string]$_) } | Select-Object -Unique)
        advertise_host = $advertiseHostResolved
        advertise_url = ("http://{0}:{1}" -f $advertiseHostResolved, $PortValue)
        reason = $Reason
        generated_at = (Get-Date).ToUniversalTime().ToString("o")
    }

    $parent = Split-Path -Parent $startupDiagnosticPath
    if (-not (Test-Path -Path $parent)) {
        New-Item -ItemType Directory -Path $parent -Force | Out-Null
    }
    $payload | ConvertTo-Json -Depth 8 | Set-Content -Path $startupDiagnosticPath -Encoding UTF8
}

function Test-TodUiProjectStatusReady {
    param([Parameter(Mandatory = $true)][int]$PortValue)

    try {
        $response = Invoke-WebRequest -UseBasicParsing -Uri ("http://localhost:{0}/api/project-status?readiness=1" -f $PortValue) -TimeoutSec 5
...
function Stop-StaleTodUiProxyForPort {
    param([Parameter(Mandatory = $true)][int]$PortValue)
    ...
    try {
        $processes = @(Get-CimInstance Win32_Process -ErrorAction Stop | Where-Object {
                ...
                $_.CommandLine -and
                $_.CommandLine -like '*tod_ui_lan_proxy.py*' -and
                $_.CommandLine -like ("*--listen-port {0}*" -f $PortValue)
            })
...
function Start-TodUiLanProxy {
    param(
        [Parameter(Mandatory = $true)][AllowEmptyString()][string]$ListenHost,
        [Parameter(Mandatory = $true)][int]$ListenPort,
        [Parameter(Mandatory = $true)][int]$TargetPort
    )

    if ([string]::IsNullOrWhiteSpace($ListenHost) -or [string]::Equals($ListenHost, "localhost", [System.StringComparison]::OrdinalIgnoreCase)) {
        return [pscustomobject]@{ started = $false; reason = "No LAN advertise host requested." }
    }
    if (-not (Test-Path -Path $todUiLanProxyScript)) {
        return [pscustomobject]@{ started = $false; reason = "LAN proxy script missing." }
    }

    $pythonCommand = $null
    foreach ($candidateName in @("python.exe", "python", "py.exe", "py")) {
        try {
            $candidate = Get-Command -Name $candidateName -ErrorAction SilentlyContinue | Select-Object -First 1
            if ($candidate -and -not [string]::IsNullOrWhiteSpace([string]$candidate.Source)) {
                $pythonCommand = [string]$candidate.Source
                break
            }
        }
        catch {
    ...
    if ([string]::IsNullOrWhiteSpace($pythonCommand)) {
        return [pscustomobject]@{ started = $false; reason = "Python runtime not found for LAN proxy." }
    }

    $outDir = Join-Path $repoRoot "tod/out"
    if (-not (Test-Path -Path $outDir)) {
        New-Item -ItemType Directory -Path $outDir -Force | Out-Null
    }
    $stdoutPath = Join-Path $outDir ("tod-ui-lan-proxy-{0}.stdout.log" -f $ListenPort)
    $stderrPath = Join-Path $outDir ("tod-ui-lan-proxy-{0}.stderr.log" -f $ListenPort)
    ...
    try {
        ...
        return [pscustomobject]@{ started = $true; process_id = $process.Id; reason = "LAN proxy started."; stdout = $stdoutPath; stderr = $stderrPath }
    }
    catch {
        return [pscustomobject]@{ started = $false; reason = $_.Exception.Message; stdout = $stdoutPath; stderr = $stderrPath }
    }
}

if (Test-TodUiProjectStatusReady -PortValue $Port) {
    $hosts = @("localhost")
    if (-not [string]::IsNullOrWhiteSpace($AdvertiseHost) -and -not [string]::Equals($AdvertiseHost, "localhost", [System.StringComparison]::OrdinalIgnoreCase)) {
        $hosts += [string]$AdvertiseHost
    }
    Write-TodUiStartupDiagnostic -Status "already_running" -PortValue $Port -AdvertiseHostValue $AdvertiseHost -ListenHosts $hosts -Reason "Existing TOD UI project-status endpoint is healthy."
    Write-Host "TOD UI already running at http://localhost:$Port/"
    exit 0
}

$staleProxyStopped = Stop-StaleTodUiProxyForPort -PortValue $Port

$listener = $null
$useLanProxy = (-not [string]::IsNullOrWhiteSpace($AdvertiseHost) -and -not [string]::Equals($AdvertiseHost, "localhost", [System.StringComparison]::OrdinalIgnoreCase))
$publicPort = $Port
$listenerStartPort = if ($useLanProxy) { $Port + 10000 } else { $Port }
$listenerPort = $listenerStartPort
$activePort = $Port
$maxPortAttempts = 15
$started = $false

for ($i = 0; $i -lt $maxPortAttempts; $i++) {
    $candidatePort = $listenerStartPort + $i
    ...
    try {
        ...
        $activePort = if ($useLanProxy) { $publicPort } else { $candidatePort }
        $started = $true
        break
    }
    catch {
...
if (-not $useLanProxy -and $activePort -ne $Port) {
    Write-Host "Requested port $Port was unavailable; using $activePort instead."
}

if ($useLanProxy) {
    Write-Host "TOD UI internal listener running at http://localhost:$listenerPort/"
    Write-Host "TOD UI public proxy target: http://localhost:$activePort/"
}
else {
...
$listenHosts = @("localhost")
if (-not [string]::IsNullOrWhiteSpace($AdvertiseHost) -and -not [string]::Equals($AdvertiseHost, "localhost", [System.StringComparison]::OrdinalIgnoreCase)) {
    $listenHosts += [string]$AdvertiseHost
}
$proxyListenHost = if ($useLanProxy) { "0.0.0.0" } else { $AdvertiseHost }
$lanProxy = Start-TodUiLanProxy -ListenHost $proxyListenHost -ListenPort $activePort -TargetPort $listenerPort
$startupReason = if ($staleProxyStopped -gt 0) { "Reclaimed stale TOD LAN proxy before startup." } else { "Started TOD UI listener." }
if ($lanProxy -and $lanProxy.started) {
    $startupReason = "$startupReason LAN proxy started for advertised host."
}
elseif ($lanProxy -and -not [string]::IsNullOrWhiteSpace([string]$lanProxy.reason) -and -not [string]::Equals([string]$lanProxy.reason, "No LAN advertise host requested.", [System.StringComparison]::OrdinalIgnoreCase)) {
    $startupReason = "$startupReason LAN proxy unavailable: $($lanProxy.reason)"
}
Write-TodUiStartupDiagnostic -Status "started" -PortValue $activePort -AdvertiseHostValue $AdvertiseHost -ListenHosts $listenHosts -Reason $startupReason
...
function Get-ObjectiveNumericValue {
    ...
    $normalized = Normalize-ObjectiveIdValue -Value $ObjectiveId
    if ([string]::IsNullOrWhiteSpace($normalized)) {
        return -1
    }

    $numericValue = -1
    if (-not [int]::TryParse($normalized, [ref]$numericValue)) {
        return -1
    }
...
function Resolve-ProjectSelectedObjectiveId {
    ...
    $watchdogIssue = if ($RecoveryWatchdog -and $RecoveryWatchdog.PSObject.Properties['last_issue']) { [string]$RecoveryWatchdog.last_issue } else { "" }
    if ([string]::Equals($watchdogIssue, 'publication_surface_divergence', [System.StringComparison]::OrdinalIgnoreCase) -and -not [string]::IsNullOrWhiteSpace($bridgeCanonicalObjectiveId)) {
...
function Get-CommunicationHealth {
    ...
    $status = "ok"
    $summary = "Communication health is currently clear."
    $pendingIssueCode = ""
    $coordinationStatus = "clear"
    $coordinationDecision = ""
    $nextStepStatus = ""
    $nextStepContinuationDecision = ""
    $nextStepDialogStatus = ""
    $nextStepStalled = $false
    $nextStepFrozen = $false
    $nextStepHandledLocally = $false
    $emergencyStatus = ""
    $emergencyIssueCode = ""

    $watchdogIssue = if ($RecoveryWatchdog -and $RecoveryWatchdog.PSObject.Properties['last_issue']) { [string]$RecoveryWatchdog.last_issue } else { "" }
    if ([string]::Equals($watchdogIssue, 'publication_surface_divergence', [System.StringComparison]::OrdinalIgnoreCase)) {
        $status = "warning"
        $pendingIssueCode = "publication_surface_divergence"
        $coordinationStatus = "publisher_realign_pending"
        $coordinationDecision = "republish_live_task_request"
        $canonical = if ($BridgeStatus -and $BridgeStatus.PSObject.Properties['canonical_mim_objective_id']) { [string]$BridgeStatus.canonical_mim_objective_id } else { "" }
        $published = if ($BridgeStatus -and $BridgeStatus.PSObject.Properties['task_request_objective_id']) { [string]$BridgeStatus.task_request_objective_id } else { "" }
        $summary = "Canonical MIM objective $canonical is ahead of the live task publication surface $published; publisher realignment is pending."
    }

    $consensusPath = if ((Get-Variable -Name nextStepConsensusPath -Scope Script -ErrorAction SilentlyContinue) -and -not [string]::IsNullOrWhiteSpace([string]$script:nextStepConsensusPath)) {
        [string]$script:nextStepConsensusPath
    }
    else {
        Join-Path $repoRoot "shared_state/NEXT_STEP_CONSENSUS.latest.json"
    }
    $policyPath = if ((Get-Variable -Name nextStepPolicyPath -Scope Script -ErrorAction SilentlyContinue) -and -not [string]::IsNullOrWhiteSpace([string]$script:nextStepPolicyPath)) {
        [string]$script:nextStepPolicyPath
    }
    else {
        Join-Path $repoRoot "shared_state/NEXT_STEP_POLICY.latest.json"
    }
    $dialogDir = if ((Get-Variable -Name dialogDirPath -Scope Script -ErrorAction SilentlyContinue) -and -not [string]::IsNullOrWhiteSpace([string]$script:dialogDirPath)) {
        [string]$script:dialogDirPath
    }
    else {
        Join-Path $repoRoot "runtime/shared/dialog"
    }
    $emergencyRequestPath = if ((Get-Variable -Name listenerEmergencyRequestPath -Scope Script -ErrorAction SilentlyContinue) -and -not [string]::IsNullOrWhiteSpace([string]$script:listenerEmergencyRequestPath)) {
        [string]$script:listenerEmergencyRequestPath
    }
    else {
        Join-Path $repoRoot "runtime/shared/TOD_MIM_EMERGENCY_REQUEST.latest.json"
    }
    $emergencyAckPath = if ((Get-Variable -Name listenerEmergencyAckPath -Scope Script -ErrorAction SilentlyContinue) -and -not [string]::IsNullOrWhiteSpace([string]$script:listenerEmergencyAckPath)) {
        [string]$script:listenerEmergencyAckPath
    }
    else {
        Join-Path $repoRoot "runtime/shared/MIM_TOD_EMERGENCY_ACK.latest.json"
    }

    $emergency = Read-JsonFileIfExists -Path $emergencyRequestPath
    $emergencyAck = Read-JsonFileIfExists -Path $emergencyAckPath
    if ($emergency -and $emergency.PSObject.Properties['status'] -and [string]::Equals([string]$emergency.status, 'active', [System.StringComparison]::OrdinalIgnoreCase)) {
        $ackResolved = $false
        if ($emergencyAck -and $emergencyAck.PSObject.Properties['status']) {
            $ackStatus = ([string]$emergencyAck.status).Trim().ToLowerInvariant()
            $ackResolved = @('resolved', 'closed', 'completed', 'accepted') -contains $ackStatus
        }
        if (-not $ackResolved) {
            $status = "critical"
            $emergencyStatus = "pending"
            $emergencyIssueCode = if ($emergency.PSObject.Properties['issue_code']) { [string]$emergency.issue_code } else { "emergency_coordination_active" }
            $summary = "Emergency coordination is active for $emergencyIssueCode."
        }
    }

    $consensus = Read-JsonFileIfExists -Path $consensusPath
    $policy = Read-JsonFileIfExists -Path $policyPath
    if ($consensus) {
        $nextStepStatus = if ($consensus.PSObject.Properties['status']) { [string]$consensus.status } else { "" }
        $sessionId = ""
        if ($consensus.PSObject.Properties['mim_position'] -and $consensus.mim_position -and $consensus.mim_position.PSObject.Properties['session_id']) {
            $sessionId = [string]$consensus.mim_position.session_id
        }
        if ($policy -and $policy.PSObject.Properties['continuation'] -and $policy.continuation -and $policy.continuation.PSObject.Properties['decision']) {
            $nextStepContinuationDecision = [string]$policy.continuation.decision
        }
        if ($policy -and $policy.PSObject.Properties['applied'] -and [bool]$policy.applied -and $policy.PSObject.Properties['provisional'] -and [bool]$policy.provisional) {
            $nextStepHandledLocally = $true
        }

        if ([string]::Equals($nextStepStatus, 'pending_mim', [System.StringComparison]::OrdinalIgnoreCase) -and -not $nextStepHandledLocally) {
            $generatedAt = if ($consensus.PSObject.Properties['generated_at']) { [string]$consensus.generated_at } else { "" }
            $ageMinutes = -1.0
            try {
                if (-not [string]::IsNullOrWhiteSpace($generatedAt)) {
                    $ageMinutes = ([DateTimeOffset]::UtcNow - ([DateTimeOffset]::Parse($generatedAt))).TotalMinutes
                }
            }
            catch {
                $ageMinutes = -1.0
            ...
            if ($ageMinutes -ge 5) {
                $status = "warning"
                $nextStepStalled = $true
                $summary = "Next-step consensus is pending MIM response beyond the acknowledgement window."
            }

            if (-not [string]::IsNullOrWhiteSpace($sessionId)) {
                $dialogPath = Join-Path $dialogDir ("MIM_TOD_DIALOG.session-{0}.latest.json" -f $sessionId)
                $dialog = Read-JsonFileIfExists -Path $dialogPath
                if ($dialog -and $dialog.PSObject.Properties['status']) {
                    $nextStepDialogStatus = [string]$dialog.status
                    if ([string]::Equals($nextStepDialogStatus, 'timed_out', [System.StringComparison]::OrdinalIgnoreCase)) {
                        $status = "critical"
                        $nextStepFrozen = $true
                        $summary = "Next-step consensus dialog timed out and requires communication recovery."
                    }
                }
            }
        }
    ...
    return [pscustomobject]@{
        status = $status
        summary = $summary
        pending_issue_code = $pendingIssueCode
        coordination_status = $coordinationStatus
        coordination_decision = $coordinationDecision
        next_step_status = $nextStepStatus
        next_step_continuation_decision = $nextStepContinuationDecision
        next_step_dialog_status = $nextStepDialogStatus
        next_step_stalled = $nextStepStalled
        next_step_frozen = $nextStepFrozen
        next_step_handled_locally = $nextStepHandledLocally
        emergency_status = $emergencyStatus
        emergency_issue_code = $emergencyIssueCode
    }
}
...
function Get-SteadyStateHealth {
    ...
    $listenerTerminal = $false
    $listenerRequestId = ""
    if ($ListenerActivity) {
        if ($ListenerActivity.PSObject.Properties['latest_request_id']) {
            $listenerRequestId = [string]$ListenerActivity.latest_request_id
        }

        $terminalCandidates = @()
        foreach ($propertyName in @('result_status', 'latest_execution_status', 'latest_cycle_classification')) {
            if ($ListenerActivity.PSObject.Properties[$propertyName]) {
                $terminalCandidates += [string]$ListenerActivity.$propertyName
            }
        }
        $listenerTerminal = @($terminalCandidates | Where-Object {
            $candidate = ([string]$_).Trim().ToLowerInvariant()
            @('completed', 'succeeded', 'already_processed', 'stale_request_ignored', 'stale_backfill_ignored') -contains $candidate
        }).Count -gt 0
    }

    if ($listenerTerminal) {
        if ($pendingCoordination) {
            $pendingRequestId = if ($coordination -and $coordination.PSObject.Properties['pending_request_id']) { [string]$coordination.pending_request_id } else { "" }
            if ([string]::IsNullOrWhiteSpace($pendingRequestId) -or -not [string]::Equals($pendingRequestId, $listenerRequestId, [System.StringComparison]::OrdinalIgnoreCase)) {
                $pendingCoordination = $false
                $coordinationStatus = "superseded_by_listener_terminal_result"
            }
        }

        if ($stallState -and $stallState.PSObject.Properties['last_request_id']) {
            $stallRequestId = [string]$stallState.last_request_id
            if (-not [string]::IsNullOrWhiteSpace($listenerRequestId) -and [string]::Equals($stallRequestId, $listenerRequestId, [System.StringComparison]::OrdinalIgnoreCase)) {
                $unchangedCycles = 0
            }
        }
    }
    ...
    if ($CadenceHealth -and $CadenceHealth.PSObject.Properties['governance'] -and $CadenceHealth.governance.PSObject.Properties['adjusted_severity']) {
        $cadenceSeverity = [string]$CadenceHealth.governance.adjusted_severity
    }
    $listenerMode = if ($UsingListenerOnly) { "listener_telemetry" } else { "state_plus_listener" }
    $watchdogState = if ($RecoveryWatchdog -and $RecoveryWatchdog.PSObject.Properties['effective_state']) { [string]$RecoveryWatchdog.effective_state } elseif ($RecoveryWatchdog -and $RecoveryWatchdog.PSObject.Properties['state']) { [string]$RecoveryWatchdog.state } else { "unknown" }
    $watchdogHealthy = [string]::Equals($watchdogState, 'healthy', [System.StringComparison]::OrdinalIgnoreCase)
    $bridgeSmoke = if ($RecoveryWatchdog -and $RecoveryWatchdog.PSObject.Properties['bridge_smoke']) { $RecoveryWatchdog.bridge_smoke } else { $null }
    $bridgeSmokeHealthy = $false
    $bridgeSmokeFailed = $false
    if ($bridgeSmoke) {
        if ($bridgeSmoke.PSObject.Properties['passed']) {
            try { $bridgeSmokeHealthy = [bool]$bridgeSmoke.passed } catch { $bridgeSmokeHealthy = $false }
            $bridgeSmokeFailed = -not $bridgeSmokeHealthy
        }
        elseif ($bridgeSmoke.PSObject.Properties['classification']) {
            $bridgeSmokeHealthy = [string]::Equals([string]$bridgeSmoke.classification, 'pass', [System.StringComparison]::OrdinalIgnoreCase)
            $bridgeSmokeFailed = -not $bridgeSmokeHealthy
        }
    }
    $liveTelemetryHealthy = $watchdogHealthy -and $bridgeSmokeHealthy -and -not [string]::Equals($cadenceSeverity, 'critical', [System.StringComparison]::OrdinalIgnoreCase) -and ($loopIdleSec -lt 300 -or $loopIdleSec -lt 0)
    $regressionReportStale = $false
    ...
    elseif ($regressionAvailable -and $failed -gt 0) {
        if ($liveTelemetryHealthy -and -not $bridgeSmokeFailed -and -not $pendingCoordination -and $unchangedCycles -eq 0) {
            $status = "ok"
            $regressionReportStale = $true
            $summary = "Regression failures are stale and superseded by healthy live bridge, cadence, and watchdog telemetry."
        }
        else {
            $status = "critical"
            $summary = "Regression failures remain and live operational telemetry shows active risk."
        }
    }
    ...
    return [pscustomobject]@{
        ...
        regression_report_stale = $regressionReportStale
        ...
        watchdog_bridge_smoke = $bridgeSmoke
        loop_idle_sec = $loopIdleSec
        cadence_severity = $cadenceSeverity
...
function Get-ProjectStatusPayload {
    ...
    elseif (Test-ShouldUseLightweightStateBus) {
        try {
            $engineeringSignal = Invoke-LightweightUiAction -Action "get-engineering-signal"
        }
        catch {
            $engineeringSignal = [pscustomobject]@{
                available = $false
                error = "Lightweight engineering signal unavailable: $($_.Exception.Message)"
            }
        }
    }
    else {
        $engineeringSignal = [pscustomobject]@{
            available = $false
            error = "Engineering signal skipped on project-status to keep UI readiness responsive; use the get-engineering-signal action for full diagnostics."
        }
    }
    ...
    while ($listener.IsListening) {
        ...
        if ($request.HttpMethod -eq "POST" -and $path -eq "/api/run") {
            try {
                ...
                Write-UiCrashLog ("[API-RUN-ERROR] " + $_.Exception.ToString())
        ...
        if ($request.HttpMethod -eq "POST" -and $path -eq "/api/operator-chat") {
            try {
                ...
                Write-UiCrashLog ("[OPERATOR-CHAT-ERROR] " + $_.Exception.ToString())
                $errorPayload = [pscustomobject]@{
                    ok = $false
                    error = $_.Exception.Message
                }
        ...
        if ($request.HttpMethod -eq "GET" -and $path -eq "/api/project-status") {
            try {
                $readiness = [string]$request.QueryString["readiness"]
                if ($readiness -in @('1', 'true', 'yes')) {
                    $payload = [pscustomobject]@{
                        ok = $true
                        status = 'ready'
                        generated_at = (Get-Date).ToUniversalTime().ToString("o")
                    }
                    Write-JsonResponse -Response $response -StatusCode 200 -Json ($payload | ConvertTo-Json -Depth 4)
                    continue
                }
                $objectiveId = [string]$request.QueryString["objective_id"]
                ...
                Write-UiCrashLog ("[PROJECT-STATUS-ERROR] " + $_.Exception.ToString())
                $errorPayload = [pscustomobject]@{
                    ok = $false
                    error = $_.Exception.Message
                }
        ...
        $response.Close()
            ...
            # Per-request outer safety net â€” log and try to return 500 so server keeps running
