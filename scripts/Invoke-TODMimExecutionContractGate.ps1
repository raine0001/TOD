param(
    [string]$OutputPath = 'tod/out/results-v2/tod-mim-execution-contract.latest.json',
    [string]$MarkdownOutputPath = 'tod/out/results-v2/tod-mim-execution-contract.latest.md',
...
$testSuites = @(
    [pscustomobject]@{
        ...
        path = 'tests/TOD.TODMimExecutionSimulation.Tests.ps1'
        purpose = 'Synthetic request-trigger-ack-result lane coverage for accepted, duplicate, stale, superseded, and wrong-target execution semantics.'
    }
)
...
$suiteOutputDir = Join-Path $outputDir 'tod-mim-execution-contract-suites'
...
$report = [pscustomobject]@{
    ...
    source = 'tod-mim-execution-contract-gate-v1'
    label = 'TOD<->MIM execution contract'
    scope = [pscustomobject]@{
        synthetic_only = $true
        contract_lane = 'listener_execution_only'
        live_runtime_validation = $false
        authority_boundary = 'dialog and adjudication contract remains validated separately'
    }
    coverage = [pscustomobject]@{
        ...
        excluded = @(
            ...
            'dialog routing and adjudication semantics',
            'production-like shared root mutation'
        )
    }
    canonical_command = '.\scripts\Invoke-TODMimExecutionContractGate.ps1 -EmitJson'
...
$markdownLines = @(
    '# TOD MIM Execution Contract Gate',
    '',
...
Write-Host ('TOD_MIM_EXECUTION_CONTRACT_PATH={0}' -f $OutputPath)
Write-Host ('TOD_MIM_EXECUTION_CONTRACT_MARKDOWN_PATH={0}' -f $MarkdownOutputPath)
Write-Host ('TOD_MIM_EXECUTION_CONTRACT_STATUS={0}' -f $(if ($gatePassed) { 'pass' } else { 'fail' }))
...
if ($FailOnFailure -and -not $gatePassed) {
    throw ('TOD<->MIM execution contract gate failed. See artifact: {0}' -f $resolvedOutputPath)
}