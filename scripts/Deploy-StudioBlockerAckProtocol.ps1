...
function Send-RemoteFileB64 {
    ...
    $remoteB64 = "/tmp/codex_studio_ack_$uploadId.b64"
    ...
    Invoke-RemoteChecked $SessionId "base64 -d '$remoteB64' > '$RemotePath' && rm -f '$remoteB64'" | Out-Null
}
...
try {
    Send-RemoteFileB64 $session.SessionId "runtime_remote_training/remote_scripts/apply_studio_blocker_ack_protocol_v1.py" "$remoteRoot/scripts/apply_studio_blocker_ack_protocol_v1.py"
    (Invoke-RemoteChecked $session.SessionId "cd '$remoteRoot' && .venv/bin/python scripts/apply_studio_blocker_ack_protocol_v1.py").Output
    (Invoke-RemoteChecked $session.SessionId "cd '$remoteRoot' && .venv/bin/python -m py_compile core/routers/studio.py").Output
    ...
    (Invoke-RemoteChecked $session.SessionId "sha256sum '$remoteRoot/core/routers/studio.py'").Output
}
finally {
